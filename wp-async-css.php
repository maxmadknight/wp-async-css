...
class WP_Async_CSS {
    ...
    function __construct()
    {
        ...
        load_plugin_textdomain( 'wp-async-css', false, dirname( plugin_basename(__FILE__) ) . '/languages/' );
    ...
    {
        // Get loadCSS-file
        $loadcss_file = $this->plugin_url . 'assets/scripts/loadCSS.min.js';
    ...
    public function cache_stylesheet_handles()
    {
        ...
        update_option('wp_async_css_stylesheet_handles', $parsed_styles);
    ...
    public function parse_styles($raw_styles)
    {
        ...
        foreach($raw_styles as $style_handle)
        {
            ...
            if(isset($registered[$style_handle]))
            {
    ...
    {   
        ...
        $whitelisted_handles = array_keys(get_option('wp_async_css_whitelisted_stylesheet_handles', array()));
    ...
    public function get_cached_stylesheet_handles()
    {
        ...
        $all_handles = get_option('wp_async_css_stylesheet_handles', array());
    ...
    public function add_options_page()
    {
        ...
        add_options_page( 'WP Async CSS', 'WP Async CSS', 'manage_options', 'wp-async-css', array($this, 'options_page_view'));
    ...
    function register_settings()
    {
        ...
        register_setting( 'wp-async-css-settings-group', 'wp_async_css_whitelisted_stylesheet_handles' );
    }
    ...
    public function options_page_view()
    {
        ...
        <div class="wrap">
            ...
            <div style="background:white;border:1px solid #eeeeee;float:right;padding:15px;max-width:200px;">
                <p style="margin-left: 6px;"><?php _e('If you found this plugin useful and want to support it you can donate with PayPal here :)', 'wp-async-css'); ?></p>
            ...
            <form method="post" action="options.php">
                <p>
                    <?php _e('Here you can select which stylesheets you want to load asynchronously.', 'wp-async-css'); ?><br>
                    <?php if($has_handles) _e('Note: If you cannot find the handle you are looking for please visit your frontpage to update the handle-list.', 'wp-async-css'); ?>
                </p> 
                <?php if($has_handles): ?>
                <?php settings_fields( 'wp-async-css-settings-group' ); ?>
                <?php do_settings_sections( 'wp-async-css-settings-group' ); ?>
                <table class="form-table" style="width:auto;clear:left;">
                    <tr>
                        <th scope="row"><?php _e('Select handles', 'wp-async-css'); ?></th>
                        <td>
                        	<fieldset>
                                <legend class="screen-reader-text"><span><?php _e('Select handles', 'wp-async-css'); ?></span></legend>
                                <?php foreach($this->all_handles as $handle): ?>
                                <label><input type="checkbox" name="wp_async_css_whitelisted_stylesheet_handles[<?php echo $handle; ?>]" value="1"<?php if(in_array($handle, $this->whitelisted_handles)) echo ' checked="checked"'; ?> /> <span class="date-time-text format-i18n"><?php echo $handle; ?></span></label><br />
                                <?php endforeach; ?>    
                        	</fieldset>
                        </td>
                    </tr>
                ...
                <?php submit_button(); ?>
                <?php else: ?>
                <p><strong><?php _e('Please navigate to your frontpage to fetch the list of available handles.', 'wp-async-css'); ?></strong></p>
                <?php endif; ?>
            </form>
